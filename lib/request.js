...
function sendSubscribeMsg(webSocket, topic, payload) {
  webSocket.send(JSON.stringify({ topic: "subscribe", payload }));
}

function subscribe(webSocket, whenOpen, name, value) {
  return async (topic, handler) => {
    await whenOpen;
    sendSubscribeMsg(webSocket, "subscribe", { name, value, topic });
